...
example {m n : ℕ} (coprime_mn : m.Coprime n) : m ^ 2 ≠ 2 * n ^ 2 := by
  intro sqr_eq
  have em : 2 ∣ m := even_of_even_sqr ⟨n ^ 2, sqr_eq⟩
  obtain ⟨k, meq⟩ := dvd_iff_exists_eq_mul_left.mp em
  ...
  have : 2 * k ^ 2 = n ^ 2 := by
    simp at this
    exact this
  have en : 2 ∣ n := even_of_even_sqr ⟨k ^ 2, this.symm⟩
  have : 2 ∣ m.gcd n := dvd_gcd em en
  have : 2 ∣ 1 := by
    simp [coprime_mn] at this
  norm_num at this
